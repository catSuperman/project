...
#define DBCfilepath  "test.dbc"

char *dbc_cfg_rdbuff = NULL;     //dbc read buffer
char dbc_rdbuff[100] ="0";
...
{
    memset(dbc_rdbuff,0,100);
    strcpy(dbc_rdbuff,rdbuf);
    ...
    /*SG_name*/
    memset(tmp,0,50);
    strncpy(tmp,(dbc_rdbuff+flag[1][0]), (flag[1][1] - flag[1][0] +1));
    ...
    memset(tmp,0,50);
    strncpy(tmp,(dbc_rdbuff+flag[3][0]), (flag[3][1] - flag[3][0] +1));
    #ifdef DEBUGPARSE
    ...
    memset(tmp,0,50);
    memset(tmp_start_size,0,30);
    strncpy(tmp,(dbc_rdbuff+flag[4][0]), (flag[4][1] - flag[4][0] +1));
    //#ifdef DEBUGPARSE
    #ifdef DEBUGPARSE
    printf("/*factor*/%s ",tmp);
    #endif // DEBUGPARSE
    ...
    /*Get Max & Min*/
    strncpy(tmp,(dbc_rdbuff+flag[5][0]), (flag[5][1] - flag[5][0] +1));
    //#ifdef DEBUGPARSE
    #ifdef DEBUGPARSE
    ...
    /*Get Unit*/
    strncpy(tmp,(dbc_rdbuff+flag[6][0]), (flag[6][1] - flag[6][0] +1));
    //#ifdef DEBUGPARSE
    ...
    /*Get receiver*/
    strncpy(tmp,(dbc_rdbuff+flag[7][0]), (flag[7][1] - flag[7][0] +1));
    //#ifdef DEBUGPARSE
    ...
    memset(tmp,0,50);
    memset(tmp_start_size,0,30);
    strncpy(tmp,(dbc_rdbuff+flag[8][0]), (flag[8][1] - flag[8][0] +1));
    while(tmp[n] != '\0')
    {
        tmp_start_size[m] = tmp[n];
        n++;
    ...
    tmp_start_size[m+1] = '\0';
    sg_ptr->signal_type = atoi(tmp_start_size);
    ...
    strncpy(tmp,(dbc_rdbuff+flag[9][0]), (flag[9][1] - flag[9][0] +1));
    while(tmp[n] != '\0')
    {
        tmp_start_size[m] = tmp[n];
        n++;
    ...
    tmp_start_size[m+1] = '\0';
    sg_ptr->signal_identifying = atoi(tmp_start_size);
    //printf("signal_identifying : %d\n",sg_ptr->signal_identifying);
    ...
    memset(tmp,0,50);
    memset(tmp_start_size,0,30);
    strncpy(tmp,(dbc_rdbuff+flag[10][0]), (flag[10][1] - flag[10][0] +1));
    while(tmp[n] != '\0')
    {
        tmp_start_size[m] = tmp[n];
        n++;
     ...
     tmp_start_size[m+1] = '\0';
    sg_ptr->realtime_show_flag= atoi(tmp_start_size);
...
int Get_SG_Index(FILE * fptr,char * rdbuf,BO_Unit_t *boptr)
{
    ...
    while(1)
    {
        ...
        if(0 == (strncmp(rdbuf,SGFLAG,5))) //DETECT THE SG_ while the end of THE hole SG_ of one BO_
        {
            ...
            SG_t *tmpsg = Parse_Line3(rdbuf,10);
...
int DBC_load(FILE * dbc_cfg_fd,char *dbc_cfg_rdbuf,BO_Unit_t **BO_List)
{
    ...
    for(y = 0;y < BO_counter; y++)
    {
        ...
        while(BO_List[y]->SG_List[i] != NULL)
        {
            ...
            printf("-------------------SG INF--------------------------\n");
            printf("SG_Name:%s  start_bit:%d  size:%d  byte_order:%d  value_type:%d\n factor:%lf offset:%lf Min:%lf Max:%lf unit:%s receiver:%s signaltype:%d signalIdentification:%d  RealTimeFlag:%d \n",
            ...
            BO_List[y]->SG_List[i]->maximum,BO_List[y]->SG_List[i]->unit,BO_List[y]->SG_List[i]->receiver,BO_List[y]->SG_List[i]->signal_type,BO_List[y]->SG_List[i]->signal_identifying,BO_List[y]->SG_List[i]->realtime_show_flag);
            i++;
        }
        printf("\n=======================BO INF========================================\n");
    }
...
{
    int dbc_falg = 0; 
    dbc_cfg_rdbuff = malloc(100);
    memset(BO_List,0,4*50);

    
    if(NULL == (dbc_cfg_fd = fopen("test.dbc","r+b+w")))
    {
    ...
    {
        DBC_load(dbc_cfg_fd,dbc_cfg_rdbuff,BO_List);   //load DBC_file
